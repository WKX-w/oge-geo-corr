...
export const oriImgTileSourceFetcher = async (rasterId: number): Promise<TileSourceResponse> => {
    return (
        await fetch(
            `${SERVICE_PREFIX}/oge-model-service/api/correction/wmts/image?ImageId=${rasterId}`
        )
    ).json();
};
...
export function useTileSource(raster: DataSource, pollingInterval?: number, onFailed?: () => void) {
    ...
    const { data: leftImgTileSource } = useSWR(rasterId.toString(), oriImgTileSourceFetcher, {
    ...
    if (
        paramsRef.current.failed === false &&
        (leftImgTileSource?.status === "failed" || rightImgTileSource?.status === "failed")
    ) {
    ...
    return {
        tileSources: {
            left: { loading: !sourceLoadedRef.current.left, url: leftImgTileSource?.result?.url },
            right: {
